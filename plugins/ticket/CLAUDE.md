...
any `ticket-*/SKILL.md`, check that a new instruction would make sense across
stacks; if not, rewrite it as a principle or move it to the per-project
`CLAUDE.md` template in `ticket-init/SKILL.md`'s Appendix.
...
For individual skill details, see each `skills/ticket-*/SKILL.md`.
